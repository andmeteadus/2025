...
Lisa raporti lõppu tabel, mis kasutab staatilise faili *Aktsiad 2022.csv* asemel reaalajas andmeid, mis on saadud portaalist *finance.yahoo.com*.

Mõned näpunäited:

-   kasuta R-i paketti `tidyquant`
-   kasuks tuleb funktsioon `tq_get`
-   hinnana kasuta tunnust *close*, sümbolina *symbol*, tootlus arvutada (kasutades eelmise kauplemispäeva sulgemishinda, kasuks võivad tulla funktsioonid `first`, `last` ja `nth`), kogusena *volume*, kasumeid ja dividende aktsiate kohta tidyquant ilma API-ta ei anna, seetõttu ei pea ostusoovitusi lisama
